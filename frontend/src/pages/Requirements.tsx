...
const requirements: RequirementCard[] = [
  {
    ...
    sources: ['Unfallatlas'],
    justification: 'Uses SQL MIN(year) across all imported accident records.',
  },
  {
    title: 'Personal injury accidents in Saxony (2023)',
    endpoint: '/accidents/count?state_ags=14&year=2023',
    sources: ['Unfallatlas'],
    justification: 'Uses SQL COUNT() filtered by Saxony AGS and year 2023.',
  },
  {
    title: 'Earliest year available for North Rhine-Westphalia',
    endpoint: '/time/earliest?state_ags=05',
    sources: ['Unfallatlas'],
    justification: 'Uses SQL MIN(year) filtered by AGS prefix 05.',
  },
  {
    title: 'Earliest year available for Mecklenburg-Western Pomerania',
    endpoint: '/time/earliest?state_ags=13',
    sources: ['Unfallatlas'],
    justification: 'Uses SQL MIN(year) filtered by AGS prefix 13.',
  },
  {
    title: 'Pedestrian accidents in Berlin (2023)',
    endpoint: '/accidents/count?state_ags=11&year=2023&ist_fuss=true',
    sources: ['Unfallatlas'],
    justification: 'Counts accidents involving pedestrians in Berlin during 2023.',
  },
  {
    title: 'District accident rates per 100,000 inhabitants',
    endpoint: '/aggregates/rates?year=2023&level=district&top_n=10',
    sources: ['Unfallatlas', 'GENESIS Destatis'],
    justification: 'Combines accident counts and population data to calculate rates.',
  },
  {
    title: 'Compare calculated rates with Regionalstatistik',
    endpoint: '/aggregates/rate-comparison',
    sources: ['Unfallatlas', 'GENESIS Destatis', 'Regionalstatistik'],
    justification: 'Compares calculated accident rates with published Regionalstatistik indicators.',
  },
];

export function Requirements(): ReactElement {
  ...
  useEffect(() => {
    const load = async () => {
      const entries = await Promise.all(
        requirements.map(async (req): Promise<[string, unknown]> => {
          try {
            const response = await api.get<ApiResponse<unknown>>(req.endpoint);
            return [req.endpoint, response.data.data];
          } catch {
  ...
  return (
    <main style={{ maxWidth: 960, margin: '0 auto', padding: '3rem 5vw' }}>
      ...
      <p style={{ maxWidth: 760, lineHeight: 1.7, marginBottom: 24 }}>
        This page demonstrates all mandatory project questions and cross-source aggregations.
      </p>
      ...
      {requirements.map((req) => (
        ...
        >
          ...
          <p>
            <strong>Sources:</strong> {req.sources.join(', ')}
          </p>
          ...
          <details>
            ...
            >
              {JSON.stringify(results[req.endpoint], null, 2)}
            </pre>
          </details>
        </section>
      ))}
